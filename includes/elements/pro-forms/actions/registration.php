...
class Registration
{
    ...
    public function run($form)
    {
        ...
        $user_email           = isset($form_settings['registrationEmail']) ? $form->get_form_field_by_id($form_settings['registrationEmail']) : false;
        $user_login           = isset($form_settings['registrationUserName']) ? $form->get_form_field_by_id($form_settings['registrationUserName']) : false;
        $user_pass            = isset($form_settings['registrationPassword']) ? $form->get_form_field_by_id($form_settings['registrationPassword']) : false;
        ...
        if (is_wp_error($registration_errors)) {
            ...
            // If the email address is already registered, we stop here
            wp_send_json_error([
                "message" => $registration_errors->get_error_message()
            ]);
        ...
        // Update first name
        if (isset($form_settings['registrationFirstName'])) {
            $first_name = $form->get_form_field_by_id($form_settings['registrationFirstName']);
            if ($first_name) {
        ...
        // Update last name
        if (isset($form_settings['registrationLastName'])) {
            $last_name = $form->get_form_field_by_id($form_settings['registrationLastName']);
            if ($last_name) {
